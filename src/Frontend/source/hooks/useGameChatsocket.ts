...
export function useGameChatWebSocket({ currentChat, onclicked }: UseGameChatWebSocketProps) {
  ...
  const handleWebSocketMessage = useCallback((event: MessageEvent) => {
    ...
    if (data.type === 'game_invite' && data.action === 'receive') {
      ...
      inviteDiv.className = 'fixed top-4 right-4 bg-primary-dark text-white p-4 rounded-lg shadow-lg';
      inviteDiv.innerHTML = `
  ...
  const handleInviteResponse = useCallback((inviteId: string, accepted: boolean) => {
